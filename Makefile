...
do/test: do/build-test
	...
	docker run -e S6_LOGGING=1 $(IMAGE_TEST):$(VERSION)-$(BUILD_NUM) bats /tests/image
	docker run -e S6_LOGGING=1 --dns=8.8.4.4 --dns-search=10.0.0.1.xip.io $(IMAGE_TEST):$(VERSION)-$(BUILD_NUM) bats /tests/dnsmasq/01-resolve_dns.bats
	docker run -e S6_LOGGING=1 --dns 8.8.4.4 --dns 8.8.8.8 --dns-search google.com --dns-search video.google.com $(IMAGE_TEST):$(VERSION)-$(BUILD_NUM) bats /tests/dnsmasq/02-resolve_dns_multiple.bats

do/build-test:
	docker build -t $(IMAGE_TEST):$(VERSION)-$(BUILD_NUM) -f tests/Dockerfile-$(VERSION) .
